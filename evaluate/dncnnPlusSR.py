...
# ================= SIZE FIX =================
def match_size(img, target):
    h, w = target.shape[:2]
    return cv2.resize(img, (w, h), interpolation=cv2.INTER_CUBIC)

def safe_downscale(img, scale=4, window=8):
    h, w = img.shape[:2]

    new_h = max(8, h // scale)
    new_w = max(8, w // scale)

    new_h = (new_h // window) * window
    new_w = (new_w // window) * window

    new_h = max(window, new_h)
    new_w = max(window, new_w)
...
def main():
    ...
    #final = match_size(final, clean)
    # Resize clean image to match SR output (not the other way!)
