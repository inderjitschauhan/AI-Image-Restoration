...
def plot_publication_figure(df):
    ...
    for sigma in sigmas:
        ...
        fig.suptitle(f"Model Comparison (sigma = {sigma})")

        axes[0, 0].bar(models, df_sigma["PSNR"])
        axes[0, 0].set_title("PSNR")
        axes[0, 0].tick_params(axis='x', rotation=45)

        axes[0, 1].bar(models, df_sigma["SSIM"])
        axes[0, 1].set_title("SSIM")
        axes[0, 1].tick_params(axis='x', rotation=45)

        axes[0, 2].bar(models, df_sigma["Time"])
        axes[0, 2].set_title("Time")
        axes[0, 2].tick_params(axis='x', rotation=45)

        axes[1, 0].bar(models, df_sigma["Params (M)"])
        axes[1, 0].set_title("Params (M)")
        axes[1, 0].tick_params(axis='x', rotation=45)

        axes[1, 1].bar(models, df_sigma["Size (KB)"])
        axes[1, 1].set_title("Size (KB)")
        axes[1, 1].tick_params(axis='x', rotation=45)

        score = (df_sigma["PSNR"]/df_sigma["PSNR"].max() +
                 df_sigma["SSIM"]/df_sigma["SSIM"].max()) / 2

        axes[1, 2].bar(models, score)
        axes[1, 2].set_title("Score")
...
def main():
    ...
    swin_size = get_model_size_kb(SWIN_SR_PATH)
