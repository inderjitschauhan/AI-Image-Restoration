...
# ================= ZOOM =================
def get_zoom_patch(img, x, y, size):
    h, w = img.shape[:2]
...
def show_all(clean, noisy, dncnn, final,
    ...
    h, w = clean.shape[:2]
    zoom_x = w // 2 - zoom_size // 2
    zoom_y = h // 2 - zoom_size // 2

    scale_x = final.shape[1] / w
    scale_y = final.shape[0] / h

    zoom_x_sr = int(zoom_x * scale_x)
    zoom_y_sr = int(zoom_y * scale_y)
    zoom_size_sr = int(zoom_size * scale_x)

    # -------- INFO --------
    w0, h0, mp0 = get_image_info(clean)
    w1, h1, mp1 = get_image_info(final)
    ...
    for i in range(4):
        ...
        if i < 3:
            ...
            rect = patches.Rectangle((zoom_x_sr, zoom_y_sr),
                                     zoom_size_sr, zoom_size_sr,
                                     linewidth=2, edgecolor='red', facecolor='none')
    ...
    for i in range(4):
        if i < 3:
            patch = get_zoom_patch(imgs[i], zoom_x, zoom_y, zoom_size)
        else:
            patch = get_zoom_patch(imgs[i], zoom_x_sr, zoom_y_sr, zoom_size_sr)

        patch = cv2.resize(patch, (256,256), interpolation=cv2.INTER_NEAREST)

        plt.subplot(1,4,i+1)
        plt.imshow(patch)
        plt.title(["Original","Noisy","DnCNN","Final"][i])
        plt.axis("off")
...
def main():
    ...
    w, h, mp = get_image_info(clean)
    print(f"\nInput Image: {w} x {h} | {mp:.2f} MP")
    ...
    print("\nBest DnCNN:", best_psnr)

    dncnn_size, dncnn_params = get_model_size(best_model)
    print(f"DnCNN Size: {dncnn_size:.2f} MB | Params: {dncnn_params/1e6:.2f}M")

    # -------- SWIN --------
    swin = load_swin_model(SWIN_PATH)

    swin_size, swin_params = get_model_size(swin)
    print(f"SwinIR Size: {swin_size:.2f} MB | Params: {swin_params/1e6:.2f}M")
    ...
    wf, hf, mpf = get_image_info(final)
    print(f"Output Image: {wf} x {hf} | {mpf:.2f} MP")
