...
# ===============================
Tk().withdraw()
image_path = askopenfilename(title="Select HR Image")
...
# HR -> LR
# ===============================
lr_image = hr_image.resize((hr_w // SCALE, hr_h // SCALE), Image.BICUBIC)
...
plt.imshow(sr_image)
plt.axis("off")
plt.text(10, 20, f"Params: {total_params/1e6:.2f}M", color='yellow', fontsize=10, backgroundcolor='black')
plt.text(10, 40, f"Inference: {(end-start)*1000:.2f} ms", color='yellow', fontsize=10, backgroundcolor='black')
plt.text(10, 70, f"PSNR: {psnr.item():.2f} dB\nSSIM: {ssim.item():.4f}", color='cyan', fontsize=10, backgroundcolor='black')
