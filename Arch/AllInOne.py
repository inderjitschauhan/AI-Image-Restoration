...
def get_random_image(folder):
    ...
    # ✅ Added: filename extraction (no logic change)
...
def add_real_noise(img):
    noise = np.random.normal(0, 0.08, img.shape)  # strong noise
    noisy = img + noise
...
def main():
    ...
    print("\n===== DEBUG =====")
    print("Noise Enabled:", USE_NOISE)
    print("Noise diff:", np.mean(np.abs(clean - noisy)))
    print("Allclose:", np.allclose(clean, noisy))
    ...
    print("\n=========== RESULTS ===========")
    print("Noisy :", calculate_psnr(clean, noisy), calculate_ssim(clean, noisy))
    print("DnCNN :", calculate_psnr(clean, best_dncnn), calculate_ssim(clean, best_dncnn))
    print("Final :", calculate_psnr(clean_resized, final), calculate_ssim(clean_resized, final))
